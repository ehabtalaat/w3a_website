  ...
  <div class="card card-custom gutter-b">
    ...
    <div class="card-header">
      <div class="card-title">
        ...
        <h3 class="card-label"> {{__('messages.edit podcast')}}</h3>
      </div>
    </div>
        ...
        <div class="row">
          <div class="col-6 mx-auto">
            <div class="uploadOuter">
              ...
              @if($podcast->image)
              <img src="{{$podcast->image->image_link}}">
              @endif
              </div>
          </div>
        <div class="col-md-6 col-12 mx-auto text-center mb-5 set-img mx-auto">
                    <video width="200" height="200" controls >
              <source src="{{ $podcast->audio_link ?? ''}}" id="video_here">
            Your browser does not support HTML5 video.
          </video>
          <br>
          <br>
                   <input id="kt" type="file"  class="form-control ehabtalaat file_upload" name="audio">
                            <label for="kt" class="custom_file_upload"> {{__('messages.add audio')}}</label>
                            @error('audio')
...
@enderror" required value="{{ $podcast->translate($localeCode)->title}}" name="title-{{ $localeCode }}">
         ...
         <div class="row">
                ...
                @error('tag_ids') is-invalid @enderror"  id="tag_id"
                                  name="tag_ids[]"   data-live-search="true" required
                                  title="&nbsp;"  multiple>
                                  @foreach ($tags as $tag)
                                  <option value="{{$tag->id}}" @slected(in_array($tag->id,$podcast->tags->pluck("id")->toArray()))
                                      >{{$tag->title}}</option>
                                  @endforeach
              
                          </select>
                          @error('tag_ids') <span class="invalid-feedback">
              {{ $message }}</span> @enderror
                      </div>
                  </div>
         </div>
          ...
          <div class="col-md-6 col-sm-12">
            <div class="form-group">
              ...
              <textarea class="form-control   @error('text-' . $localeCode) is-invalid 
                          @enderror" required name="text-{{ $localeCode }}" rows="6">{{$podcast->translate($localeCode)->text ?? ''}}
              </textarea>
