...
<div class="container">
  <div class="card card-custom gutter-b">
    ...
    <div class="card-body">
      <form method="post" action="{{route('doctor_offline_times.update',$doctor->id)}}" enctype="multipart/form-data">
        @csrf
  ...
  <div class="card_working appointment{{$day->id}}">
    ...
    <div class="form-check">
      <input class="form-check-input check_day{{$day->id}}" name="day_ids[]"
       onchange="check_times({{$day->id}})" @checked(in_array($day->id,$doctor->doctor_offline_days->pluck("day_id")->toArray()))
    ...
    @foreach($doctor->doctor_offline_days()->where("day_id",$day->id)->first()->times ?? [] as $key1 => $time_doctor)
