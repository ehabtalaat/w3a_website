...
<div class="card card-custom gutter-b">
    ...
    <div class="card-header">
		<div class="card-title">
			...
			<h3 class="card-label"> {{__('messages.edit doctor')}}</h3>
		</div>
	</div>
    ...
    <div class="row">
        ...
        @if($doctor->image)
        <img src="{{$doctor->image->image_link}}">
        @endif
        </div>
              </div>
...
<div class="row">
    ...
    <div class="col-md-6 col-sm-12">
        <div class="form-group">
            <label>
            {{ __('messages.mini_description_'.$localeCode) }}
                <span class="text-danger"> ( {{ $localeCode }} )</span>
            </label>
            <textarea  class="form-control  @error('mini_description-' . $localeCode) is-invalid @enderror"
            name="mini_description-{{ $localeCode }}" rows="3"
             placeholder="{{ __('messages.mini_description_'.$localeCode) }}" required>
             {{$doctor->translate($localeCode)->description ?? ""}}
            </textarea>
             @error('mini_description-' . $localeCode) <span class="invalid-feedback">
    {{ $message }}</span> @enderror
        </div>
    </div>
    @endforeach
            ...
            <div class="row">
                ...
                <div class="col-md-6 col-sm-12">
                    <div class="form-group">
                        ...
                        name="description-{{ $localeCode }}" rows="7"
                         placeholder="{{ __('messages.description_'.$localeCode) }}" required>
                         {{$doctor->translate($localeCode)->description ?? ""}}
                        </textarea>
                         @error('description-' . $localeCode) <span class="invalid-feedback">
                {{ $message }}</span> @enderror
                    </div>
                </div>
                @endforeach
